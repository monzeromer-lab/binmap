...
how `binmap` discovers what it is looking at.
...
| Approach | Assessment |
|---|---|
| **DAP (Debug Adapter Protocol)** | **Recommended.** JSON over stdio, well-specified, stable. `lldb-dap` and CodeLLDB both speak it. It is designed exactly for a GUI driving a debugger, which is what Binmap is. |
...
|---|---|---|---|
| **GPUI + `gpui-component`** | **High** | Pre-1.0, frequent breaking changes, forked ecosystem — and now the only product surface | Pin exact; isolate in `binmap-gui`; scheduled upgrades (`DESIGN-GUI.md` §7); engine stays independently testable |
...
headlessly by `binmap-eval`, so a bad GPUI upgrade cannot stall analysis work. `rr` is Phase 4
...
checks are available headlessly via `binmap-eval doctor` for CI.
