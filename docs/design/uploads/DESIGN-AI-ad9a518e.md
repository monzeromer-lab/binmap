# Binmap — AI Agent Design

> How Binmap uses language models: two modes, one tool registry, one trust boundary.
...
Binmap supports AI in two ways, and the difference is not cosmetic — it is a difference in
**who owns the loop.**

| | **Mode A — Native agent** | **Mode B — ACP** |
|---|---|---|
| Model access | Direct API, user's key | External agent process |
| Who runs the loop | Binmap | The external agent |
| Who chooses tools | Binmap's registry | The agent, from what we expose |
| Who manages context | Binmap | The agent |
...
**Mode A is a controlled loop.** Binmap decides what the model sees, forces a hypothesis
before every tool call, enforces a step budget, and refuses to construct a finding that cites
no evidence.

**Mode B is an inversion.** Binmap becomes the *client* and the agent becomes the reasoner.
The agent has its own harness, its own context strategy, its own model, and its own billing.
Binmap cannot make Codex state a hypothesis before calling a tool, because Codex's loop is
Codex's business.

What Binmap retains in Mode B is narrower but still decisive: **it owns the tools, and it
owns the airlock.** The agent can only learn about the binary through tools Binmap wrote,
...
**Step 2 is the load-bearing one for Binmap.** Because the client supplies MCP server
configurations when creating a session, Binmap can hand the external agent its entire
...
Binmap is Rust and GPUI is from the same organisation, prefer the official Rust crate
...
└─────────────┬─────────────┘        └─────────────────┼─────────────────┘
              ...
              └────────────────┬───────────────────────┘
                               ▼
              ┌────────────────────────────────┐
              │   TOOL REGISTRY (binmap-agent)   │   ← single source of truth
              ...
              └────────────────┬───────────────┘
                               ▼
              ┌────────────────────────────────┐
              │  Analysis engines · binmap-verify│
              └────────────────────────────────┘
```
...
`binmap-mcp` wraps the same registry as an MCP server and hands its address to the agent at
`session/new`.

**Transport choice.** MCP over stdio is conventional, but Binmap is the parent process and
...
tiny `binmap-mcp-bridge` helper binary that proxies stdio to the local HTTP endpoint.
...
Mode B agents emit prose. Binmap needs findings. Two mechanisms, used together:
...
- **Inferred (native)** — Binmap's own loop, hypothesis-gated, budget-bounded
...
Binmap's own reasoning, because the accountability differs.
...
- A project-level `binmap.toml` may set `allow_cloud_models = false`, and when it does, the
...
Binmap's careful slicing applies only to what individual tools return. Claude Code decides
...
file writes, and terminal commands through the client. Binmap is the client and therefore
...
installs and Binmap executes. Show the resolved command before first launch, pin versions
...
| 2 | **Mode B**: ACP client, `binmap-mcp` server, Claude Code and Codex. Permission mapping to trust tiers. |
...
1. **Does `session/new`'s MCP configuration support HTTP servers across all target agents**,
   or is a stdio bridge required for some? Determines whether `binmap-mcp-bridge` is needed.
...
4. **Should Binmap itself ship as an ACP agent**, so that Zed or JetBrains could drive
   Binmap's analysis from their own agent panels? A genuinely interesting inversion, and
