...
built on GPUI, and three of its components map almost exactly onto Binmap's hardest views:
...
- **A virtualized data table.** Built for large datasets with virtual scrolling, resizable
  ...
  handling hundreds of thousands of rows. Binmap's symbol table is tens of thousands of
...
- **A code editor.** Stable at around 200K lines, with tree-sitter syntax highlighting and
  LSP-backed diagnostics, completion, and hover. Binmap needs a *viewer*, not an editor,
...
- The forks' selling point is features Zed rejects — custom shaders and similar. Binmap
...
```
binmap-gui  ──depends on──>  binmap-core (types + Engine trait)
                           binmap-session (read sessions/artifacts)

binmap-gui  ──MUST NOT depend on──>  binmap-analyze, binmap-agent, binmap-acp, binmap-binary
```

Enforced in CI with `cargo-deny`'s dependency bans, not by convention.

**Two — the headless eval harness.** `binmap-eval` drives every engine feature without a window.
A feature that only works when the UI is running fails the eval suite. This is the substitute
for the forcing function the CLI used to provide, and it is the reason `binmap-eval` exists at
...
The Binmap-specific part is **inline frame expansion**. A physical stack frame in an
...
2. **Isolate the surface.** All GPUI usage lives in `binmap-gui`. No other crate imports it.
   Every custom widget goes in `binmap-gui::widgets`, so a framework API change touches one
...
Window, theme, dock layout, project view, Profile Lab reading `binmap.json`, and the
