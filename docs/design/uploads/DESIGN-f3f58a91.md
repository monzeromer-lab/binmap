...
**The `lodestone.json` boundary is load-bearing**, and more so now that the GUI is the only
...
│   ├── lode-agent/       Tool registry, native agent loop, model backends
│   ├── lode-acp/         ACP client — external agents (Claude Code, Codex)
│   ├── lode-mcp/         MCP server exposing the tool registry to those agents
│   ├── lode-session/     Session store, artifact serialisation, replay
│   ├── lode-eval/        Headless harness. Dev-only; never published or shipped.
│   └── lode-gui/         GPUI application — the only product surface
...
Lodestone supports two AI modes, and the difference is a difference in *who owns the loop*.

**Mode A — native agent.** Lodestone drives the model directly through a user-supplied API key.
...
**Mode B — ACP.** Lodestone becomes a *client* and an external agent (Claude Code, Codex CLI)
becomes the reasoner. The agent owns its loop, its context strategy, its model, and its
billing. Lodestone exposes its analysis tools to that agent through a loopback MCP server
...
Lodestone's slicing applies only to individual tool outputs.
...
and no system webview dependency. `lode-gui` links the engine crates directly.
...
**Rule:** `lode-gui` depends on `lode-core` and `lode-session` only. It reaches analyses
exclusively through a facade trait defined in `lode-core`:
...
The Cargo dependency graph enforces this — `lode-gui` cannot call an analysis engine because
it does not depend on one. `lodestone.json` remains the serialisation of everything crossing
...
file writes, and terminal commands through the client, and Lodestone is the client. Scope
...
- The suite is driven by `lode-eval` headlessly, never through the GUI. An engine feature that
    ...
    └── lode-session─┘                 │
              ...
              lode-binary (DWARF) ─────┼── Phase 2: crash analysis── SHIP
                        ...
                        └──────────────┼── Phase 3: perf attrib   ── SHIP
                                       │
              lode-agent (Mode A) ─────┼── Phase 1 also: native AI ── SHIP
              lode-acp + lode-mcp ─────┼── Phase 2 also: ACP        ── SHIP
...
| Lodestone as an ACP *agent* | Post-v1 | Would let Zed or JetBrains drive Lodestone's analysis. Possibly better distribution than a standalone GUI. Keep the tool registry clean enough to allow it. |
