# Binmap — Harness Toolchain Reference

> Working reference for `clap`, `hyperfine`, and `bloaty` — the CLI surface and the two
> measurement tools Binmap shells out to. Companion to `TOOLING-BINARY.md`.

---

## 1. The headless harness (`binmap-eval`)
...
quietly growing a dependency on the UI. That job belongs to `binmap-eval`: a development-only
...
```rust
#[derive(Parser)]
#[command(name = "binmap-eval", about = "Development harness. Not a product surface.")]
struct Cli {
...
#[derive(Subcommand)]
enum Command {
    /// Run one analysis and emit binmap.json
...
1. **stdout is data, stderr is progress.** `binmap-eval` emits only `binmap.json` on stdout.
...
| Former command | Now |
|---|---|
| `binmap doctor` | Environment panel (PRD U10). Same checks, same exact-fix messages, rendered rather than printed. |
| `binmap tune` | Profile Lab |
| `binmap size` | Size Explorer |
| `binmap analyze <core>` | Crash view, with a file picker. **See PRD open question 1** — this is the one case that plausibly warrants a thin launcher, since core dumps originate in terminals. |
| `binmap profile` | Flamegraph view |
| `binmap verify` | The Verify button on any proposal in the Findings Inspector |
| `binmap ci` | **Dropped.** PRD R13 records the cost: this was the cheapest path into an organisation's repository. Revisit after Phase 2 as a separate crate. |
...
machinery. It is also a good `binmap doctor` check: a machine with a 15% noise floor should be
...
fn attribution_reconciles_with_bloaty() {
    for binary in corpus_binaries() {
        let ours   = binmap_size::attribute(&binary).unwrap();
...
1. **Version-check at discovery**, not at first use. `binmap doctor` surfaces the result.
